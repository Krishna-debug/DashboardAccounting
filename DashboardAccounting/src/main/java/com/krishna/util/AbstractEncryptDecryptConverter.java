...
public abstract class AbstractEncryptDecryptConverter<X>  implements AttributeConverter<X, String> {
	...
	private static String SECRET_NEW_ENCRYPTION_KEY ;
	...
	public void  setKey(String key) {
		AbstractEncryptDecryptConverter.SECRET_ENCRYPTION_KEY=key;
	}
	
	@Value("${com.oodles.accounts.new.key}")
	public void  setNewKey(String key) {
		AbstractEncryptDecryptConverter.SECRET_NEW_ENCRYPTION_KEY=key;
	}
	...
	@Override
	 public String convertToDatabaseColumn(X attribute) {
	  if (isNotEmpty(SECRET_NEW_ENCRYPTION_KEY) && isNotNullOrEmpty(attribute)) {
	   try {
	    Cipher cipher = cipherMaker.configureAndGetInstance(
	      Cipher.ENCRYPT_MODE, 
	      SECRET_NEW_ENCRYPTION_KEY);
